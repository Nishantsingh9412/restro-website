...
export const addEmployee = async (req, res) => {
	try {
		const requiredFields = ["name", "email", "employeeID", "status", "type", "created_by"];
		...
		return res.status(200).json({ success: true, message: "Employee created successfully", result: newEmployee });
	} catch (error) {
...
export const getEmployeesByRestaurant = async (req, res) => {
	try {
		...
		const query = key === "userId" ? { created_by: value } : key === "id" ? { _id: value } : {};

		const employees = await Employee.find(query);
		if (!employees.length) {
			return res.status(404).json({ success: false, message: "No employees found for this restaurant" });
		}
		return res.status(200).json({ success: true, message: "Employees retrieved successfully", result: employees });
	} catch (error) {
...
export const updateEmployee = async (req, res) => {
	try {
		...
		return res.status(200).json({ success: true, message: "Employee updated successfully", result: employee });
	} catch (error) {
...
export const getTodaysBirthday = async (req, res) => {
	try {
		...
		return res.status(200).json({ success: true, message: "Birthdays today", result: employees });
	} catch (error) {
...
export const getUpcomingEmployeeBirthday = async (req, res) => {
	try {
		...
		if (!upcomingBirthdays.length) {
			return res.status(404).json({ success: false, message: "No employees have birthdays in the next 30 days." });
		}

		return res.status(200).json({ success: true, message: "Upcoming birthdays", result: upcomingBirthdays });
	} catch (error) {
