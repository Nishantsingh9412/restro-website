...
export const getSingleOrderItem = async (req, res) => {
  ...
  try {
    ...
    if (!GetSingleOrderItemByID) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to get Order Item" });
    } else {
      return res.status(200).json({
        success: true,
        message: "Single Order Item",
        result: GetSingleOrderItemByID,
      });
    }
  } catch (err) {
...
export const getAllOrderItems = async (req, res) => {
  ...
  try {
    const allOrderItems = await OrderdItems.find({
      isDrink: { $ne: true },
      created_by: _id,
    }).sort({ isFavorite: -1, orderName: 1 });
    // const allOrderItems = await OrderdItems.find({ isFavourite: -1, orderName: 1 });
    if (!allOrderItems) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to get Order Items" });
    } else {
      return res.status(200).json({
        success: true,
...
export const getDrinksOnly = async (req, res) => {
  ...
  try {
    const getDrinks = await OrderdItems.find({
      isDrink: true,
      created_by: _id,
    }).sort({ isFavorite: -1, orderName: 1 });
    if (!getDrinks) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to get Drinks" });
    } else {
      return res
        .status(200)
        .json({ success: true, message: "All Drinks", result: getDrinks });
    }
  } catch (err) {
...
export const updateOrderItem = async (req, res) => {
  ...
  try {
    ...
    const updateOrderSingleItem = await OrderdItems.findByIdAndUpdate(
      _id,
      {
        $set: {
          orderName: orderName,
...
export const deleteOrderItem = async (req, res) => {
  ...
  try {
    ...
    if (!deleteSingleOrderItem) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Deleted Order Item" });
    }
  } catch (err) {
...
export const searchOrderItems = async (req, res) => {
  ...
  try {
    ...
    if (!searchOrderItem) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to search Order Items" });
    } else {
      return res.status(200).json({
        success: true,
        message: "Searched Order Items",
...
export const searchDrinksOnly = async (req, res) => {
  ...
  try {
    ...
    if (!searchDrinks) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to search Drinks" });
    } else {
      return res.status(200).json({
        success: true,
        message: "Searched Drinks",
        result: searchDrinks,
      });
    }
  } catch (err) {
