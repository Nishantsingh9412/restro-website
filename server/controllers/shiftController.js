...
export const editEmployeeShift = async (req, res, next) => {
  try {
    ...
    if (!_id) return res.status(400).json({ success: false, message: "Shift ID is required" });

    const fromDate = parseDateTime(date, from);
    const toDate = parseDateTime(date, to);
    const duration = moment.duration(moment(toDate).diff(moment(fromDate))).asHours();
    ...
    return res.status(200).json({ success: true, message: "Shift edited", result: shift });
  } catch (err) {
    next(err);
  }
};
...
export const deleteEmployeeShift = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ message: "Shift deleted successfully.", success: true });
  } catch (err) {
    next(err);
  }
};
...
export const addShiftOfEmployee = async (req, res, next) => {
  try {
    ...
    const shift = new Shift({ employeeId, date, from: fromDate, to: toDate, duration, note });
    await shift.save();

    return res.status(201).json({ message: "Shift added", success: true, result: shift });
  } catch (error) {
    next(error);
  }
};
...
export const getEmployeeShift = async (req, res, next) => {
  try {
    ...
    const shifts = await Shift.find({ employeeId }).sort({ from: 1 }).populate("employeeId", "name");

    if (!shifts.length) return res.status(404).json({ message: "No shifts found for this employee." });

    return res.status(200).json({ message: "Shift fetched", success: true, result: shifts });
  } catch (error) {
    next(error);
  }
};
...
export const getTodaysShift = async (req, res, next) => {
  try {
    ...
    const shifts = await Shift.find({ from: { $gte: todayStart }, to: { $lte: todayEnd } }).populate("employeeId", "name");
...
export const getCurrentMonthShifts = async (req, res, next) => {
  try {
    ...
    const shifts = await Shift.find({ employeeId, from: { $gte: startOfMonth }, to: { $lte: endOfMonth } })
      .sort({ from: 1 })
      .populate("employeeId", "name");

    if (!shifts.length) return res.status(404).json({ message: "No shifts found for this employee in the current month." });
...
export const getShiftByIdAndDate = async (req, res, next) => {
  try {
    ...
    const shift = await Shift.findOne({ employeeId: new mongoose.Types.ObjectId(employeeId), date: shiftDate });

    if (!shift) return res.status(404).json({ message: "Shift not found" });

    return res.status(200).json({ message: "Shift fetched", success: true, result: shift });
  } catch (error) {
    next(error);
  }
};
...
export const getEmployeesWithShiftsByUser = async (req, res, next) => {
  try {
    ...
    if (!employees.length) return res.status(404).json({ message: "No employees found", success: false });
    ...
    return res.status(200).json({ message: "Shift fetched", success: true, result: employeesWithShifts });
  } catch (error) {
    next(error);
  }
};
