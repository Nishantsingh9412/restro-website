...
export default function AllOrders() {
  ...
  const autoFillform = () => {
    ...
    const randomPriceVal =
      priceVals[Math.floor(Math.random() * priceVals.length)];
    const randomPriceUnit =
      priceUnits[Math.floor(Math.random() * priceUnits.length)];
  ...
  const handleSubmitItemOrder = (e) => {
    ...
    const AddItemPromise = dispatch(AddOrderItemAction(newItemOrderData)).then(
      (res) => {
        if (res.success) {
          onClose();
          return res.message;
  ...
  const handleRemoveItemOrder = (id) => {
    ...
    if (id !== undefined) {
      setAllOrderTotal(
        allOrderTotal -
          selectedItemTemp.find((item) => item._id === id).priceVal
      );
  ...
  const handleRemoveItemOrderCompletely = (id) => {
    ...
    if (id !== undefined) {
      setAllOrderTotal(
        allOrderTotal -
          selectedItemTemp.find((item) => item._id === id).priceVal *
            selectedItemTemp.find((item) => item._id === id).quantity
      );
  ...
  return (
    <div style={{ marginTop: "4vw" }}>
      ...
      <Box display={"flex"} justifyContent={"center"} marginLeft="12rem">
        <Button
          leftIcon={<FiPlusCircle />}
          ml={"10px"}
          colorScheme="cyan"
