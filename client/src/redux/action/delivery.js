...
export const getActiveDeliveryAction = (id) => async (dispatch) => {
  try {
    const { data } = await api.getActiveDelivery(id);
    dispatch({ type: "SET_ACTIVE_DELIVERY", data: data?.result });
    return {
      success: true,
      message: "Active Delivery Fetched Successfully",
    };
  } catch (err) {
    console.log("Error from getActiveDeliveries Action: " + err?.message);
    return { success: false, message: "something went wrong" };
  }
};
